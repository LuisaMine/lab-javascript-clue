...
const suspectsArray = [];
suspectsArray.push(mcGreen, drOrchid, profPlum, missScarlet, mrsPeacock, mrMustard);
...
// ITERATION 2
//CREAR UNA FUNCION PARA SELECCIONAR ALEATORIAMENTE

function selectRandom(arr) {

  let indexAleatorio = arr[Math.floor(Math.random() * arr.length)];
    return array[indexAleatorio];
}

function pickMystery(result) {

   let asesino = selectRandom(suspectsArray);
   let cuarto = selectRandom(roomsArray);
   let arma = selectRandom(weaponsArray);

   const result = [];
      result.push(asesino,cuarto,arma);
      return result;
...
function revealMystery(finalResult) {
    return finalRsult(result.asesino).firstName + 'killed Mr. Boddy usinng the' + finalResult(result.arma).weapon + 'in the final' + finalResult(result.cuarto) + '!'; 
...
if (typeof module !== 'undefined') {
 module.exports = {
   suspectsArray: suspectsArray.asesino,
   roomsArray: roomsArray.cuarto,
   weaponsArray: weaponsArray.arma,
   pickMystery: result.pickMystery,
   revealMystery: finalResult.revealMystery,
   selectRandom: arr.selectRandom,
 };
}